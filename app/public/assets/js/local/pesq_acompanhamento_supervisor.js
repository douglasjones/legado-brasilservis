...
function fcGeralRelatorio(){
    ...
    sendPost("relatorio","receptivoAcompanhamentoSupervisor",objParametros);
...
function fcCarregarColaborador() {
    ...
    var arrCarregar = carregarController("colaborador", "listarTodosAtivo", objParametros);
    var arrCarregar = carregarController("colaborador", "listarTodosAtivo", objParametros);

    // Remove as opções antigas
    $("#colaborador_pk").empty();

    // Adiciona novas opções
    $.each(arrCarregar, function (index, item) {
        $("#colaborador_pk").append(
            $("<option>", { value: item.pk, text: item.ds_colaborador })
        );
    });

    // Atualiza o Chosen depois de popular o select
    $("#colaborador_pk").trigger("chosen:updated");
