...
class SlimImage {
    ...
    public static function getImages($inputName = 'slim') {
        ...
        $values = SlimImage::getPostData($inputName);
        ...
        foreach ($values as $value) {
            $inputValue = SlimImage::parseInput($value);
            if ($inputValue) {
    ...
    private static function parseInput($value) {
        ...
        if (isset ($data->input)) {
            $inputData = isset($data->input->image) ? SlimImage::getBase64Data($data->input->image) : null;
            $input = array(
                'data' => $inputData,
        ...
        if (isset($data->output)) {
            $outputData = isset($data->output->image) ? SlimImage::getBase64Data($data->output->image) : null;
            $output = array(
                'data' => $outputData,
