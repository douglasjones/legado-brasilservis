...
class CategoriaProduto {
    ...
    public function listarGrid($ds_categoria, $ic_status){
        ...
        $sql.="       ,CASE  WHEN ic_status=1 THEN 'Ativo' ELSE 'Desativado' END t_ic_status  ";
        $sql.="       ,ic_status ";
    ...
    public function salvar($categoria_produto){
        ...
        $fields = array();
        $fields['ds_categoria'] = $categoria_produto['ds_categoria'];
        $fields['ic_status'] = $categoria_produto['ic_status'];
        
        $fields["dt_ult_atualizacao"] = "sysdate()";
        $fields["usuario_ult_atualizacao_pk"] = $_SESSION['session_user']['par1'];

        if($categoria_produto['pk']  == ""){

            $fields["dt_cadastro"] = "sysdate()";
            $fields["usuario_cadastro_pk"]   = $_SESSION['session_user']['par1'];


            $pk = Util::execInsert("categorias_produto", $fields,$this->pdo);
            $retorno->status = true;
            $retorno->message = 'Dados cadastrados com sucesso';
            $retorno->data = $pk;
        }
        else{
            Util::execUpdate("categorias_produto", $fields, " pk = ".$categoria_produto['pk'],$this->pdo);
            $pk = $categoria_produto['pk'];
            $retorno->status = true;
    ...
    public function excluir($pk){
        Util::execDelete('categorias_produto', ' pk='.$pk, $this->pdo);
    }
